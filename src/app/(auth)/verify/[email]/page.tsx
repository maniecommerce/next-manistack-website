...
export default function OTPVerifyProGUI() {
  ...
  function focusIndex(i: number) { const node = inputsRef.current[i]; if (node) node.focus(); }

  function handleChange(ch: string, idx: number) {
    if (!/^\d?$/.test(ch)) return;
    setValues((v) => { const copy = [...v]; copy[idx] = ch; return copy; });
  ...
  function handleKey(e: React.KeyboardEvent<HTMLInputElement>, idx: number) {
    if (e.key === "ArrowLeft") { e.preventDefault(); focusIndex(Math.max(0, idx - 1)); }
    if (e.key === "ArrowRight") { e.preventDefault(); focusIndex(Math.min(length - 1, idx + 1)); }
    if (e.key === "Backspace" && !values[idx] && idx > 0) { e.preventDefault(); focusIndex(idx - 1); }
  }
  ...
  async function handleVerify() {
    ...
    if (code.length !== length) { setMessage("Enter full 6-digit code"); return; }
    ...
    <div className="bg-white/6 rounded-2xl p-6 sm:p-8 lg:p-12 shadow-xl backdrop-blur-md">
      <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <img src={MOODBOARD} className="w-14 sm:w-16 lg:w-20 h-14 sm:h-16 lg:h-20 object-cover rounded-md" />
          <div>
            <div className="text-sm sm:text-base text-gray-300">Verify your email</div>
            <div className="font-semibold text-lg sm:text-xl lg:text-2xl">{email}</div>
          </div>
        </div>
      ...
      <div className="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
        <div>
          <div className="text-sm sm:text-base text-gray-300 mb-2">Enter verification code</div>
          <div className="flex flex-wrap gap-2 lg:gap-3" onPaste={handlePaste}>
            {values.map((val,i) => (
              <input
                key={i}
                inputMode="numeric"
  ...
  >
    {loadingSend ? "Sending..." : cooldown > 0 ? `Resend in ${cooldown}s` : "Resend Email"}
  </button>
  <button
  ...
  >
    {loadingVerify ? "Verifying..." : "Verify"}
          ...
          {message && <div className={`mt-3 text-sm ${status==="error"?"text-rose-300":"text-green-200"}`}>{message}</div>}
        </div>

        <div>
          <div className="text-sm sm:text-base text-gray-300">Help & Security</div>
          <div className="mt-3 text-sm sm:text-base text-gray-400 space-y-2">
            <p>If you did not receive the email, check your spam folder or try resending after the cooldown.</p>
            <p>Your OTP is valid for 5 minutes. Do not share with anyone.</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="mt-6 text-center text-xs sm:text-sm text-gray-400">
      Need other verification methods? Contact support or use SMS verification.
    </div>
  </div>
</div>
